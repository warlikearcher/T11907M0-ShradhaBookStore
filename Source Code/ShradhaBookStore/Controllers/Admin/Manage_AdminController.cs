	...
	public class Manage_AdminController : Controller
	{
		...
		public IActionResult Manage_Admin()
        {
            var linq = from a in context.Product
                       ...
                       group new { a, i.CategoryName, } by new
                       {
                           a.CategoryCode,
                           Column1 = i.CategoryName
                       } into g
                       select new
                       {
                           g.Key.Column1,
            ...
            {
                //var getFullName = context.Category.Where(s => s.CategoryCode.Equals(item./*CategoryCode*/)); 
                dataPoints.Add(new DataPoint(item.Column1, item.cout));
